...
import aiohttp
# from lxml import etree as ET_lxml # No longer needed directly in main.py for sitemap parsing
import logging
# from datetime import datetime, timezone # No longer needed directly in main.py for sitemap parsing
from pathlib import Path
# --- End Sitemap Processing Imports ---
...
@eel.expose
async def fetch_urls_from_remote_server(server_ip: str = "192.168.0.43") -> Dict[str, Any]:
    """
    Fetches a list of URLs to crawl from a remote server's API endpoint.
    In a real scenario, this would communicate with the specified IP address.
    """
    target_url = f"http://{server_ip}:8000/get_urls_to_crawl"
    logger.info(f"Attempting to fetch URLs from remote server: {target_url}")
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(target_url, timeout=10) as response:
                if response.status == 200:
                    ...
                    logger.info(f"Fetched URLs from {server_ip}: {message}")
                    return {"status": "success", "urls": urls, "message": message}
                else:
                    error_text = await response.text()
                    logger.error(f"Failed to fetch URLs from {server_ip}. HTTP Status: {response.status}, Response: {error_text}")
                    return {"status": "error", "message": f"Failed to fetch URLs: Server responded with HTTP {response.status}. {error_text}"}
    except aiohttp.ClientConnectorError as e:
        logger.error(f"Could not connect to remote server {server_ip}: {e}")
        return {"status": "error", "message": f"Could not connect to the remote server ({server_ip}). Please ensure it is running and accessible. Error: {e}"}
    except asyncio.TimeoutError:
        logger.error(f"Timeout when trying to connect to {server_ip}")
        return {"status": "error", "message": f"Connection to remote server ({server_ip}) timed out."}
    except Exception as e:
        logger.error(f"An unexpected error occurred while fetching URLs from {server_ip}: {e}", exc_info=True)
        return {"status": "error", "message": f"An unexpected error occurred while fetching URLs: {str(e)}"}
...
def sanitize_filename(url: str) -> str:
    try:
        ...
        filename_base = re.sub(r'[\\s\\._-]+', '_', filename_base) 
        filename_base = re.sub(r'^_*', '', filename_base) 
        filename_base = re.sub(r'_*$', '', filename_base) 
...
def sanitize_dirname(url: str) -> str:
    try:
        ...
        dirname_base = re.sub(r'[\\s\\._-]+', '_', dirname_base)
        dirname_base = re.sub(r'^_*', '', dirname_base)
        dirname_base = re.sub(r'_*$', '', dirname_base)
        if not dirname_base:
                            ...
                            if page_data_for_links and page_data_for_links.success and page_data_for_links.links:
                                ...
                                for link_info in page_data_for_links.links.get("internal", []): 
                                    ...
                                    try:
                                        ...
                                        if parsed_norm_abs_link.scheme in ('http', 'https') and \
                                           parsed_norm_abs_link.netloc == expected_domain and \
                                           norm_abs_link not in crawled_urls and norm_abs_link not in queued_urls:
                    ...
                    if page_data.success and current_depth < max_depth and page_data.links:
                        for link_info in page_data.links.get("internal", []): 
                            ...
                            try:
                                ...
                                if parsed_normalized_absolute_url.scheme in ('http', 'https') and \
                                   parsed_normalized_absolute_url.netloc == expected_domain: 
...
async def start_crawl_process(
    ...
    """
    Initiates the web crawling process for multiple URLs using Eel.
    ...
    try:
        ...
        _is_processing = False
        return {"status": "error", "message": f"Failed to create base output directory {output_dir}: {str(e)}"}
...
@eel.expose
async def notify_server_crawl_finished(server_ip: str = "192.168.0.43", processed_successfully: bool = True):
    """
    Simulates notifying the remote server that processing is finished.
    In a real scenario, this would involve network communication.
    """
    target_url = f"http://{server_ip}:8000/notify_crawl_finished"
    status_message = "successfully_processed" if processed_successfully else "finished_with_issues"
    payload = {"status": status_message, "machine_name": platform.node()}
    
    logger.info(f"Simulating notifying {server_ip} at {target_url} that processing is finished (success: {processed_successfully})...")
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(target_url, json=payload, timeout=5) as response:
                response_data = await response.json()
                if response.status == 200:
                    logger.info(f"Server {server_ip} acknowledged completion: {response_data.get('message', 'Unknown response')}")
                    return {"status": "acknowledged", "message": response_data.get('message', 'Server acknowledged completion.')}
                else:
                    logger.warning(f"Server {server_ip} responded with {response.status} when notifying completion: {response_data.get('message', 'Unknown error')}")
                    return {"status": "error", "message": f"Server did not acknowledge completion: {response_data.get('message', 'Unknown error')}"}
    except aiohttp.ClientConnectorError as e:
        logger.error(f"Could not connect to remote server {server_ip} to notify completion: {e}")
        return {"status": "error", "message": f"Could not connect to remote server ({server_ip}) to notify completion: {e}"}
    except asyncio.TimeoutError:
        logger.error(f"Timeout when notifying {server_ip} of completion.")
        return {"status": "error", "message": f"Timeout when notifying remote server ({server_ip}) of completion."}
    except Exception as e:
        logger.error(f"Unexpected error when notifying {server_ip} of completion: {e}", exc_info=True)
        return {"status": "error", "message": f"Unexpected error notifying remote server: {str(e)}"}


if __name__ == "__main__":
    logger.info("Starting Eel application...")
    # Hide the default Eel console window on Windows if desired
    # import sys
    ...
    eel.init('web') # Point Eel to the web directory
    # Use a default browser, or specify one like 'chrome' or 'edge'
    # eel.start('index.html', size=(1000, 800), mode='edge') 
